...
def test_kam_newton_unbounded(k,n_gr,n,msrc,ssrc,m1,m2,s1,s2,w1,w2):
    ...
    ax.plot(x_gr, v_gr, "o", label="$v_{{}}$".format(k))
    ax.xaxis.set_tick_params(labelsize=30)
    ax.yaxis.set_tick_params(labelsize=30)
    ax.legend(fontsize=30,framealpha=0)
    ...
    tight_layout()
    savefig("../plots/hist-k{}-ub.png".format(k))	

    fig, ax = subplots()
    ax.set_xlabel("x", fontsize=30)
    ax.plot(x_gr, p_gr, "v", ms=6.0, label=R"$p_{{}}$".format(k))
    ax.plot(x_gr, q_gr, "o", ms=3.0, label=R"tar score, $q$")
    ax.xaxis.set_tick_params(labelsize=30)
    ax.yaxis.set_tick_params(labelsize=30)
    ax.legend(fontsize=30,markerscale=3,framealpha=0)
    ax.grid(True)
    ax.set_title("After {} iteration(s)".format(k),fontsize=30)
    tight_layout()
    ...
    fig, ax = subplots()
    ax.plot(x, Tx, "o", label=R"$T$")
    ax.plot(x, x, "v", label="Id")
    ax.xaxis.set_tick_params(labelsize=30)
    ax.yaxis.set_tick_params(labelsize=30)
    ax.legend(fontsize=30,markerscale=3,framealpha=0)
    ax.grid(True)
    ax.set_xlabel("x", fontsize=30)
    ax.set_ylabel(R"T(x)", fontsize=30)
    ax.set_title("After {} iteration(s)".format(k),fontsize=30)
    tight_layout()
    savefig("../plots/transport-k{}-ub.png".format(k))
   
    fig, ax = subplots()
    ax.semilogy(range(1,k+1),normv,"P--",ms=10.0,label=R"$\|v\|$")
    ax.xaxis.set_tick_params(labelsize=30)
    ax.yaxis.set_tick_params(labelsize=30)
    ax.legend(fontsize=30)
    ax.set_xlabel("Iteration number",fontsize=30)
    ax.grid(True)
    ax.set_xticks(range(1,k+1))
    tight_layout()
    savefig("../plots/normv-k{}.png".format(k))	
...
def test_kam_newton_bounded():
    ...
    ax.xaxis.set_tick_params(labelsize=24)
    ax.yaxis.set_tick_params(labelsize=24)
    ax.legend(fontsize=20,markerscale=3,framealpha=0)
    ax.grid(True)
    ax.set_title("After {} iteration(s)".format(k),fontsize=24)
    tight_layout()
